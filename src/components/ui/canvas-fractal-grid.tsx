...
const useAdaptivePerformance = (
  ...
  const [perfLevel, setPerfLevel] = useState(initialPerformance)
  ...
  useEffect(() => {
    if (fps < 30 && perfLevel !== "low") {
      setPerfLevel("low")
    } else if (fps >= 30 && fps < 50 && perfLevel !== "medium") {
      setPerfLevel("medium")
    } else if (fps >= 50 && perfLevel !== "high") {
      setPerfLevel("high")
    }
  }, [fps, perfLevel])

  return { performance: perfLevel }
}
...
export function CanvasFractalGrid({
  ...
  const { performance } = useAdaptivePerformance(initialPerformance)
